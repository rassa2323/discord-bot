...
# ── HONEYPOT CHANNELS — anyone who writes gets 1 day timeout ─────────────────
HONEYPOT_CHANNEL_IDS: set = set()
honeypot_timeout_count: int = 0
...
def load_data():
    ...
    try:
        ...
        for cid in data.get("honeypot_channels", []):
            HONEYPOT_CHANNEL_IDS.add(int(cid))
    ...
    with _save_lock:
        try:
            ...
            snapshot = {
                ...
                "honeypot_channels":   list(HONEYPOT_CHANNEL_IDS),
                "users": {
...
# ─── HONEYPOT EMBED HELPERS ──────────────────────────────────────────────────
async def send_honeypot_embed(channel: discord.TextChannel):
    embed = discord.Embed(
        title="⚠️ Honeypot Warning",
        description="**DO NOT SEND MESSAGES IN THIS CHANNEL**",
        color=discord.Color.yellow()
    )
    embed.add_field(
        name="​",
        value="This channel is used to catch spam bots. Any messages sent here will result in an immediate 1day timeout.",
        inline=False
    )
    embed.add_field(name="Timeouts", value=f"**{honeypot_timeout_count}**", inline=True)
    embed.add_field(
        name="​",
        value="If you want to get un-timed out, DM <@1482300597935013968>",
        inline=False
    )
...
async def update_honeypot_embed(channel: discord.TextChannel):
    """Edit the most recent bot embed in the channel to update the timeout count."""
    try:
        async for msg in channel.history(limit=50):
            if msg.author == channel.guild.me and msg.embeds:
                emb = msg.embeds[0]
                if emb.title and "Honeypot" in emb.title:
                    ...
                    for field in emb.fields:
                        if field.name == "Timeouts":
                            new_emb.add_field(name="Timeouts", value=f"**{honeypot_timeout_count}**", inline=True)
                        else:
                            new_emb.add_field(name=field.name, value=field.value, inline=field.inline)
                    new_emb.set_footer(text=emb.footer.text if emb.footer else SERVER_NAME)
...
async def on_message(message: discord.Message):
    ...
    if message.channel.id in HONEYPOT_CHANNEL_IDS:
        if not is_immune(member) and not is_super_immune(member):
            try:
                await message.delete()
            except (discord.NotFound, discord.Forbidden):
                pass
            try:
                await member.timeout(timedelta(days=1), reason="Wrote in honeypot channel")
            except (discord.Forbidden, discord.HTTPException):
                pass
            try:
                dm_embed = discord.Embed(
                    title="⚠️ Honeypot Warning",
                    description="You have been timed out for **1 day**.",
                    color=discord.Color.red(),
                    timestamp=discord.utils.utcnow()
                )
                dm_embed.add_field(name="Server", value=SERVER_NAME, inline=True)
                dm_embed.add_field(name="Reason", value="You sent a message in a honeypot channel. This channel exists to catch spam bots.", inline=False)
                dm_embed.set_footer(text="Do not write in channels you are not supposed to.")
                await member.send(embed=dm_embed)
            except discord.Forbidden:
                pass
            global honeypot_timeout_count
            honeypot_timeout_count += 1
            update_user_stats(member, "Honeypot", "1 day timeout")
            add_log("🍯 Honeypot Triggered", str(member), member.id, message.channel.name, message.content[:100], "1 day timeout")
            log_channel = message.guild.get_channel(CONFIG["log_channel_id"])
            if log_channel:
                embed = discord.Embed(title="🍯 Honeypot Triggered", color=discord.Color.red(), timestamp=discord.utils.utcnow())
                embed.add_field(name="User",    value=f"{member.mention} ({member})", inline=True)
                embed.add_field(name="Channel", value=message.channel.mention, inline=True)
                embed.add_field(name="Message", value=message.content[:800] or "*(empty)*", inline=False)
                embed.add_field(name="Punishment", value="1 day timeout", inline=True)
                try:
                    await log_channel.send(embed=embed)
                except discord.Forbidden:
                    pass
            # Update the pinned honeypot embed with new count
            await update_honeypot_embed(message.channel)
        return
    ...
    if is_immune(member):
        ...
        if content.strip() == "!honeypot":
            ch = message.channel
            if ch.id in HONEYPOT_CHANNEL_IDS:
                await message.channel.send(f"⚠️ #{ch.name} is already a honeypot channel.")
                return
            try:
                await ch.edit(name="honeypot")
            except (discord.Forbidden, discord.HTTPException):
                pass
            HONEYPOT_CHANNEL_IDS.add(ch.id)
            save_data()
            await send_honeypot_embed(ch)
            add_log("🍯 Honeypot Created", str(member), member.id, ch.name, f"Channel {ch.id} set as honeypot")
            return

        # ── !removehoneypot — remove honeypot from current channel ───────────
        if content.strip() == "!removehoneypot":
            ch = message.channel
            if ch.id not in HONEYPOT_CHANNEL_IDS:
                await message.channel.send(f"❌ #{ch.name} is not a honeypot channel.")
                return
            HONEYPOT_CHANNEL_IDS.discard(ch.id)
            save_data()
            await message.channel.send(f"✅ #{ch.name} is no longer a honeypot channel.")
            add_log("🍯 Honeypot Removed", str(member), member.id, ch.name, f"Channel {ch.id} removed from honeypot")
            return
